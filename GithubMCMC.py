...
def pi(beta):
    return 1#pdf_norm(beta, np.zeros(len(beta)), np.eye(len(beta)))
...
def inv_fisher(X, beta):
    w = [phi_prime(np.dot(X[i], beta)) ** 2 / (phi(np.dot(X[i], beta)) * (1 - phi(np.dot(X[i], beta)))) for i in
         range(X.shape[0])]
    W = np.diag(w)
...
def metropolis(X, Y, tau, iters):
    ...
    while i < iters:
        ...
        if counter in progress:
            print(int(counter / (iters / 100)), "%")
    return d
...
def diagnostics(indep, dep, iter, n_boostrap):
    start = timer()
    tmp = metropolis(indep, dep, 1 , iter)
    end = timer()
    ...
    for i in range(X.shape[1]):
        plt.plot(np.arange(iter - burn_in), tmp_betas[burn_in:, i], alpha=0.5, label=f'{cols[i]}',
                 color=colours[i])  # intercept
    ...
    for k in b_dict.keys():
        ...
        plt.plot([b_dist[k]['lb_cred'], b_dist[k]['ub_cred']], [0, 0], linewidth=4, label="credibility interval",
